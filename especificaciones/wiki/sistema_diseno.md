...
- **Aesthetic-Usability Effect (Efecto Estético-Usabilidad)**: La combinación del Azul Base `#324354`, el Lino Cálido `#F6F3EE`, el acento Slate Green `#7B8E90`, el cristal translúcido y la fotografía de alta hegemonía generan una percepción inmediata de sofisticación y durabilidad, incrementando la paciencia y tolerancia emocional en compras de ticket elevado ($1.000.000 a $25.000.000 COP).
- **Fitts’s Law (Ley de Fitts)**: Todo botón primario, selector de swatch o disparador táctil tiene un área mínima de interacción de 44×44 px. En mobile, la acción principal de compra permanece adherida a la zona ergonómica del pulgar mediante una Sticky Bottom Bar.
- **Doherty Threshold (Umbral de Doherty)**: Toda interacción (cambio de variante, apertura de drawer, cálculo de cuotas ADDI o añadido al carrito) proporciona feedback visual en <150 ms y completa su respuesta en <400 ms. Los estados asíncronos muestran skeletons animados inmediatos para preservar el estado de flujo.
- **Hick’s Law & Choice Overload (Ley de Hick y Sobrecarga de Elección)**: La parametrización de medidas y colores se divide en pasos secuenciales o selectores de máximo 4 a 6 variantes visibles simultáneas, evitando la parálisis por análisis.
- **Von Restorff Effect (Efecto de Aislamiento)**: El CTA de conversión principal ([Añadir al Carrito] / [Comprar Ahora]) porta el contraste cromático de alta visibilidad con resplandor focal (`--shadow-floating-cta`), desmarcándose con nitidez de la paleta neutra y arquitectónica.
- **Gestalt: Proximity & Common Region**: Espaciados matemáticos jerárquicos: la distancia entre un label y su input es 4 veces menor que la separación respecto al siguiente grupo de datos. Cada SKU o combo se auto-contiene en tarjetas con límites perceptuales definidos sobre fondo Lino `#F6F3EE`.
- **Postel’s Law (Ley de Robustez)**: Entradas flexibles en buscadores, campos de cotización y checkout: formateo automático de cédula/NIT, teléfonos celulares y tolerancia a faltas ortográficas o sinónimos en el catálogo ("tina", "jacuzzi", "hidromasaje").
...
| CÓDIGO HEX | NOMBRE / ROL | CMYK | RGB | NOTAS Y RESTRICCIONES DE USO |
| :--- | :--- | :--- | :--- | :--- |
...
> El tono Lino Cálido se destina a fondos secundarios, tarjetas y superficies orgánicas nobles. **Queda estrictamente prohibido sobresaturar la interfaz con este tono** como fondo monolítico en pantallas completas o contenedores masivos, para evitar que la plataforma pierda contraste, nitidez visual y sensación de pulcritud arquitectónica.
...
:root {
  ...
  --color-brand-linen:   #f6f3ee; /* Superficie noble secundaria (No sobresaturar) */
  ...
  --shadow-floating-cta: 0 12px 28px rgba(50, 67, 84, 0.32);
}
```
...
El sistema de diseño estipula dos familias tipográficas oficiales con roles claramente diferenciados:

- **Tipografía Primaria: Montserrat**
  - **Asignación**: Jerarquías de lectura, cuerpos de texto descriptivos, interfaz de usuario general (UI), botones transaccionales, subtítulos, etiquetas y datos de fichas técnicas.
  - **Razón de diseño**: Sans-serif moderna de alta legibilidad en pantallas táctiles y desktop, neutra y equilibrada.
- **Tipografía Secundaria: Orborn Medium**
  - **Asignación**: Encabezados específicos de marca, titulares principales de alto impacto (H1/Display), acentos visuales y números monumentales.
  - **Razón de diseño**: Familia geométrica de carácter arquitectónico distintivo que refuerza el posicionamiento premium de Firplak.

### Configuración Web e Importaciones

```html
<!-- Tipografía Primaria Oficial: Montserrat (Google Fonts) -->
...
```css
/* Tipografía Secundaria Oficial: Orborn Medium */
@font-face {
  font-family: 'Orborn';
...
:root {
  ...
  --font-secondary: 'Orborn', 'Montserrat', -apple-system, sans-serif;
...
Para respetar la Ley de Miller (7±2 unidades de información) y la Ley de Prägnanz, las fichas técnicas y textos extensos nunca se presentan en bloques planos; se agrupan en módulos semánticos de 3 a 5 especificaciones:

| NIVEL | TIPOGRAFÍA | PESO | TAMAÑO / LEADING | REGLA HEURÍSTICA / CASO DE USO |
| :--- | :--- | :--- | :--- | :--- |
| **H1 (Hero / PDP)** | `Orborn` / `Montserrat` | Medium (500) / Bold (700) | 32-44px / 1.15 | **Foco Primario**: Nombre del producto y propuesta de valor única. |
| **H2 (Secciones)** | `Orborn` / `Montserrat` | Medium (500) / Semi-Bold (600) | 24-32px / 1.25 | **Estructura Escaneable**: Títulos de fases scrollytelling y colecciones. |
| **H3 (Subtítulos)** | `Montserrat` (Primaria) | 600 (Semi-Bold) | 18-22px / 1.30 | **Chunking**: Grupos de atributos (Dimensiones, Material, Eléctrico). |
| **Metrics / Num** | `Orborn` / `Montserrat` | Medium (500) / Bold (700) | 28-36px / 1.00 | **Von Restorff**: Cifras de garantía (5 Años), Litros (350L) y Potencia (2 HP). |
| **Price Active** | `Montserrat` (Primaria) | 700 (Bold) | 24-30px / 1.10 | **Claridad Comercial**: Precio en COP con separador de miles claro. |
| **Body (Cuerpo)** | `Montserrat` (Primaria) | 400 (Regular) | 15-16px / 1.60 | **Legibilidad Óptima**: Párrafos explicativos (máx. 65-75 caracteres por línea). |
| **Technical Data** | `Montserrat` (Primaria) | 500 (Medium) | 13-14px / 1.45 | **Precisión**: Datos tabulares de instalación, voltajes y pesos. |
| **Badges / Tags** | `Montserrat` (Primaria) | 600 (Semi-Bold) | 11-12px / 1.00 | **Escaneo Rápido**: Envío Rápido <48h, Ahorras 25%, 5 Años Garantía. |
...
- **Dark Slate Glass (Uso Principal sobre Video / Fotografía Diurna)**:
  - **Fondo con Opacidad Calibrada**: `rgba(50, 67, 84, 0.60)` a `rgba(20, 27, 35, 0.70)` (basado en `--color-brand-primary`). Permite apreciar la textura, agua o ambiente que transcurre detrás.
  - **Desenfoque Óptico**: `backdrop-filter: blur(12px)` a `blur(16px)`.
...
- **Light Linen Glass (Uso Secundario sobre Escenas Nocturnas o Fondos Oscuros)**:
...
- **Ubicación Canónica Desktop (Cuadrante Inferior Izquierdo)**: Posicionamiento `justify-end items-start` con márgenes perimetrales amplios (`p-6` a `p-12`). Ancla la lectura en el ángulo natural mientras el producto luce en el centro/derecha.
- **Ubicación Alternativa (Cuadrante Inferior Derecho)**: Exclusivo para tomas donde el producto físico esté encuadrado a la izquierda.
- **Ubicación en Mobile (Bottom-Anchor / Hoja Inferior)**: Anclado al borde inferior (`bottom: 0` o margen de 16px), con ancho de `calc(100% - 32px)`. Libera el 60-65% superior del pantalla para la acción del video.
- **ZONA PROHIBIDA**: Terminantemente prohibido centrar cuadros de texto sobre el tercio medio de la pantalla. Ningún contenedor opaco o semitransparente debe ocultar el centro geométrico ni la silueta del producto.

### D. Redondeos (Border-Radius) y Proporciones Geométricas

| ELEMENTO UI | CLASE TAILWIND | RADIO (PX) | JUSTIFICACIÓN Y PROPORCIÓN |
| :--- | :--- | :--- | :--- |
| **Cuadros de Texto sobre Video** | `rounded-2xl` | 16px a 20px | Geometría arquitectónica contemporánea; suaviza el impacto visual sobre el video. |
| **Chips, Badges y Botones CTA** | `rounded-full` | 9999px (Pill) | Contraste morfológico directo. Facilita el clic y refuerza la ergonomía de Fitts ($\ge 44$px). |
| **Drawers y Bottom Sheets Móviles** | `rounded-t-3xl` | 24px a 32px | Esquinas superiores redondeadas sugerentes del despliegue desde abajo. |
| **Swatches de Materiales** | `rounded-xl` / `rounded-full` | 12px / Círculo | Muestra fiel del grano de mármol o cuarzo sin esquinas duras. |
| **Price Tags Flotantes del Cursor** | `rounded-full` | 9999px (Pill) | Micro-cápsula aerodinámica que sigue suavemente el cursor sin bloquear el producto. |

- **Ancho Máximo en Desktop**: `max-w-md` a `max-w-lg` (448px a 512px). Representa entre el 28% y el 35% del ancho del viewport; el 65-72% restante permanece como visual pura.
- **Altura Relativa**: La altura del cuadro nunca debe exceder el 40% de la altura de la pantalla.
- **Padding Interno**: `p-6` a `p-7` (24px a 28px de respiro interno).
...
1. **El Isotipo "Λ" Firplak**: Micro-marca de agua con opacidad del 20-30%, o sello formal de 16x16px en sellos de garantía (5 Años) o badges técnicos.
2. **Chips / Tag Pills Translúcidos**: Píldoras compactas (`rounded-full`) con fondo `rgba(255, 255, 255, 0.10)` o color acento `#7B8E90`, tipografía Montserrat mayúscula (`tracking-[0.2em]`).
3. **Líneas Divisorias de Tensión Mínima**: Trazos ultrafinos de 1px con opacidad reducida (`border-white/10`).
4. **Micro-Iconos de Línea Fina (Lucide trazo 1.5px)**: Iconos geométricos lineales de trazo delgado (`ArrowRight`, `ZoomIn`, `Play/Pause`).
5. **Cursor / Price Tag Flotante (Hover Tag)**: Cápsula translúcida interactiva (`rounded-full`, `bg-black/60`, `backdrop-blur-md`) con etiqueta de precio o nombre del componente.
6. **Indicadores de Progreso Lineales Ultrafinos**: Barra de avance continuo de 2px de grosor ubicada al borde superior o inferior.

### F. Elementos Gráficos Excluidos (Anti-Patrones)

- ❌ **Prohibido**: Stickers, ilustraciones vectoriales decorativas, iconos rellenos o doodles.
- ❌ **Prohibido**: Fondos sólidos y 100% opacos en cuadros flotantes sobre video.
- ❌ **Prohibido**: Sombras difusas coloreadas con tonos saturados fuera de la escala Dutch Boy 2025.
- ❌ **Prohibido**: Múltiples botones de acción dentro del mismo cuadro flotante (Ley de Hick: máximo 1 CTA primario por cuadro).
...
- **Fundamento UX**: Fitts’s Law, Von Restorff Effect y Jakob’s Law.
...
- **Contenido Ergonómico**:
  - Resumen visual en miniatura (48×48 px).
...
- **Fundamento UX**: Hick’s Law, Doherty Threshold y Law of Similarity.
...
- **Dimensiones**: Área táctil de 44×44 px (núcleo visual de 32 px con padding transparente de 6 px para Ley de Fitts).
...
- **Fundamento UX**: Selective Attention y Cognitive Load.
- Iconos minimalistas estilo Lucide (trazo 1.5px) complementados por el isotipo Λ para desarmar objeciones de compra:
...
*Detalle extendido en [pagina_inicio.md](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/pagina_inicio.md)*.

- **Announcement Bar & Goal-Gradient Effect**: Ticker rotativo con incentivos claros: "¡Envío Gratis por compras superiores a $X!" o cuotas ADDI calculadas al instante.
- **Top Bar Corporativo (Mental Model)**: Iconografía canónica (teléfono, WhatsApp, usuario) para atención inmediata y B2B.
- **Megamenú & Serial Position Effect**: Posicionamiento estratégico: categorías de mayor conversión (Hidromasajes y Baños) al inicio, y Zona Outdoor / Saldos al final.
- **Hero Cinematográfico (Aesthetic-Usability Effect)**: Video continuo con descansos en loop de 3-4s generado con Higgsfield REAL + modelo CAD real. Carga inicial optimizada con imagen WebP estática servida por CDN (LCP < 1.2s, Doherty compliant).
- **Grid Circular de Categorías (Hick's Law)**: Acceso visual instantáneo a los 6 entornos clave sin submenús anidados en primera vista.
- **Módulo Flash Sales (Von Restorff)**: Descuentos con ahorro explícito en COP (Ahorras: $ XXX.XXX), botones de compra rápida con estados de carga inmediatos.
- **Hub de Catálogos (Zeigarnik Effect)**: Acceso para diseñadores y constructores con descarga inmediata en PDF.
- **Showrooms Omnicanal (Jakob's Law)**: Enlace nativo a Google Maps y llamada directa para los 4 puntos físicos (Bogotá, Medellín e Itagüí, Cali).
- **Footer Regulatorio (Trust & Assurance)**: Respaldo legal SIC, política de entrega en primer piso y Habeas Data.

### B. Ficha de Detalle de Producto (PDP) y Scrollytelling en 8 Fases

*Detalle extendido en [pagina_producto.md](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/pagina_producto.md)*.
...
### C. Módulo Visualizador Mix & Match (Enfoque Mobile First)

*Detalle extendido en [mix_and_match.md](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/mix_and_match.md)*.

> [!IMPORTANT]
> **Enfoque de Desarrollo Mobile First**:
> El módulo Visualizador Mix & Match está conceptualizado y diseñado prioritariamente para la experiencia de compra desde dispositivos móviles (smartphones y tablets). Garantiza interacción fluida al alcance del pulgar, swipes laterales táctiles sin fricción y una secuencia de personalización en 4 pasos claros:

1. **Paso 1: Selección del modelo de lavamanos**: Exploración de tipologías (sobreponer, incrustar, vessel) con previsualización inmediata sobre el mueble.
2. **Paso 2: Opción de selección de color del lavamanos**: Conmutación de acabados del lavamanos (Blanco Brillo, Blanco Mate, Negro Satinado, tonos marfil).
3. **Paso 3: Selección del diseño del mueble**: Exploración de modelos de muebles suspendidos o de piso en madera RH.
4. **Paso 4: Opción de selección de color del mueble**: Cambio de textura y color de madera RH (Rovere, Nogal, Blanco, Duna).

- **Absorción de Complejidad en Base de Datos (Tesler’s Law)**: Supabase valida en tiempo real la compatibilidad dimensional entre el lavamanos y el mueble seleccionado, deshabilitando automáticamente combinaciones físicamente incompatibles.
- **Ley de Región Común y Conectividad Uniforme**: Los assets WebP transparentes comparten caja delimitadora idéntica, proyectando la ilusión física de un mueble de baño íntegro e instalado.
- **Venta Cruzada Orientada al Objetivo (Goal-Gradient Effect)**: Selección guiada de complementos (grifería compatible alta/baja, desagüe push y espejo LED) sumando el paquete con descuento explícito visible.
...
*Detalle extendido en [conceptos_catalogo.md](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/conceptos_catalogo.md) y [recomendaciones_pagos_y_productos.md](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/recomendaciones_pagos_y_productos.md)*.

- **Promesa de Despacho Estándar de 15 Días Hábiles**: Fabricación personalizada bajo pedido. El flag ⚡ Envío Rápido se reserva exclusivamente para inventario con despacho en <48h.
- **Restricción Estricta de Entrega en Primer Piso**: Banner visible en PDP y checkout: *"La entrega de productos voluminosos se efectúa en el primer piso del domicilio. La transportadora no realiza maniobras por fachadas o escaleras"*.
- **Plazo de 3 Días Hábiles para Reporte de Averías Estéticas**: Banner preventivo en confirmación de orden y correo post-venta informando que reclamaciones por desportillados o rayones deben reportarse dentro de los primeros 3 días hábiles.
- **Esquema Integral de Garantías de Fábrica**:
...
- **El SEO NUNCA vive dentro del Canvas o WebGL**: Todo encabezado `<h1>`, `<h2>`, tabla técnica y esquema estructurado de Schema.org (Product, Offer, AggregateRating) se genera en el servidor (SSR/ISR) mediante HTML semántico.
- **El Canvas WebGL/2D actúa como un overlay interactivo en el cliente**.
- **Presupuesto de Rendimiento**: LCP < 1.5s. Carga diferida (`next/dynamic` con `ssr: false`) en móviles para componentes WebGL.
...
- **Regla del Pulgar (Thumb Zone)**: Los elementos de navegación secundaria se colocan en el tercio superior; los elementos transaccionales primarios (Añadir al Carrito, Ver Carrito, Filtrar) se sitúan invariablemente en el tercio inferior.
- **Bottom Sheets en lugar de Modales Centrados**: Los selectores complejos (guía de medidas, cotizaciones de envío, especificaciones de pre-instalación) se abren como cajones inferiores sobre superficie Lino Cálido (`#F6F3EE`), naturales al gesto táctil.
- **Prevención de Zoom Accidental**: Tamaños de fuente en input, select y textarea fijados en un mínimo de 16px en mobile para impedir el zoom involuntario en navegadores iOS Safari / WebKit.
...
- [ ] **Ergonomía Táctil (Fitts)**: ¿Los botones y selectores miden al menos 44×44 px y están al alcance del pulgar en mobile?
...
- [Índice General del Wiki](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/index.md)
- [Página de Inicio y Navegación Global](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/pagina_inicio.md)
- [Página de Detalle de Producto (PDP) y Scrollytelling](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/pagina_producto.md)
- [Módulo Visualizador Mix & Match](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/mix_and_match.md)
- [Conceptos del Catálogo y Políticas Comerciales](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/conceptos_catalogo.md)
- [Spas e Hidromasajes](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/hidromasajes.md)
- [Cocinas Integrales y Superficies](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/cocinas.md)
- [Zona de Labores y Lavaderos](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/zona_labores.md)
- [Zona Outdoor y Asadores BBQ](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/zona_outdoor.md)
- [Accesorios y Mobiliario Complementario](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/accesorios.md)
- [Grifería y Plomería Técnica](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/griferia_plomeria.md)
- [Carpintería de Obra Institucional](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/carpinteria_obra.md)
- [Servicios Técnicos e Instalación Certificada](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/servicios.md)
- [Arquitectura de Datos y Supabase](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/arquitectura_datos.md)
- [Pagos, ePayco y Crédito ADDI](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/pagos_e_integraciones.md)
- [Especificaciones SEO y Search AI](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/seo.md)
- [Banco de Referentes Externos y Benchmarks UX](file:///c:/Users/gabriel.molina/Desktop/ecomerce_firplak/especificaciones/wiki/referentes_externo.md)
